...
/* ── AUTH-AWARE NAVBAR ──
   Runs on every page that loads main.js.
   If a user session exists, replaces the "Sign In / Get Started"
   buttons with "My Dashboard" and a "Sign Out" link so logged-in
   users always have a way back to their account.                  */
(function updateNavForUser() {
  const raw = localStorage.getItem('ee_current_user');
  if (!raw) return;               // not logged in — leave nav as-is
  ...
  const dashUrl = user.role === 'vendor' ? 'vendor-dashboard.html' : 'consumer-dashboard.html';
  const firstName = (user.name || 'Account').split(' ')[0];

  cta.innerHTML = `
    <a href="${dashUrl}" class="nav-signin" style="font-weight:700;color:var(--green-700)">
      👋 ${firstName}
    </a>
    <a href="${dashUrl}" class="btn btn-primary btn-sm">My Dashboard</a>
    <button class="btn btn-sm"
      style="background:var(--bg);color:var(--text-soft);border:1px solid var(--border);cursor:pointer"
      onclick="localStorage.removeItem('ee_current_user');window.location.reload()">
      Sign Out
    </button>
  `;
})();